...
public class PraiseCommandHandler implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        PraiseType existingSelection = voteManager.getPlayerSelection(player.getUniqueId());
        boolean isAdjustingExistingVote = existingSelection != null;
        ...
        if (!bypassCooldown && !isAdjustingExistingVote && timeSinceLastPraise < voteManager.getCooldownTime()) {
        ...
        if (!voteManager.isCurrentVote(praiseType)) {
            player.sendMessage(messageService.format("vote_in_progress",
                Map.of("vote", voteManager.getCurrentVoteDisplayName())));
            return true;
        }
    ...
    private void handleForce(CommandSender sender, String[] args) {
        ...
        boolean applied = voteManager.forceApply(praiseType);
        sender.sendMessage(messageService.format(
            applied ? "force_applied" : "force_failed",
            Map.of("vote", praiseType.getDisplayName())
        ));
    }
}
