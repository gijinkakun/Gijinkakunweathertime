...
public class VoteManager {
    ...
    private static final String PRAISE_PERMISSION = "gijinkakunweathertime.praise";
    ...
    public VoteManager(ActionBarUtils actionBarUtils, JavaPlugin plugin, FileConfiguration config, MessageService messageService) {
        ...
        this.actionBarMessenger.setRecipientFilter(this::isWorldEnabled);
    ...
    /**
     * @param playerId player whose participation should be inspected
     * @return the praise selection the player has active in the current vote, or {@code null} if none
     */
    public synchronized PraiseType getPlayerSelection(UUID playerId) {
        return playerVoteSelections.get(playerId);
    }
    ...
    /**
     * @return the display name of the active vote, or {@code "another vote"} when none is active
     */
    public synchronized String getCurrentVoteDisplayName() {
        return currentVote != null ? currentVote.getDisplayName() : "another vote";
    }

    public synchronized boolean forceApply(PraiseType vote) {
        boolean applied = executeVoteAction(vote);
        if (applied) {
            clearVotes(false);
    ...
    private int calculateRequiredVotes(int eligiblePlayers) {
        ...
        if (requiredPercentage == 50) {
            // Require a strict majority when set to 50% so 2/2 players must agree.
            required = eligiblePlayers <= 1 ? 1 : (eligiblePlayers / 2) + 1;
        } else {
            required = (int) Math.ceil(eligiblePlayers * (requiredPercentage / 100.0));
        }
    ...
    private int countEligiblePlayers() {
        return (int) Bukkit.getOnlinePlayers().stream()
            ...
            .filter(player -> player.hasPermission(PRAISE_PERMISSION))
            .count();
    }
    ...
    private boolean executeVoteAction(PraiseType vote) {
        List<World> targetWorlds = resolveTargetWorlds();
        if (targetWorlds.isEmpty()) {
            plugin.getLogger().warning("No eligible worlds configured for praise '" + vote.getCommandArgument() + "'. Skipping world updates.");
            return false;
        }
        vote.applyToWorlds(targetWorlds);
        actionBarMessenger.sendPraiseMessage(vote);
        lastSuccessfulPraise.put(vote, System.currentTimeMillis());
        return true;
    }

    private void pruneExpiredCooldowns(long currentTime) {
        if (cooldownTime <= 0) {
            if (!playerCooldowns.isEmpty()) {
                playerCooldowns.clear();
            }
            lastCooldownPrune = currentTime;
            return;
        }
        if (currentTime - lastCooldownPrune < cooldownTime) {
            return;
        }
        long cutoff = currentTime - cooldownTime;
